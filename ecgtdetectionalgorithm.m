...
for k=1:length(r_peak_pos_last)
    mt=a2(r_peak_pos_last(k):r_peak_pos_last(k)+ceil(fs/bpm)*5);
    mn=min(mt);
...
for m=1:length(r_peak_pos_last)
    mt2=a2(r_peak_pos_last(m)-(ceil(fs/bpm)*5):r_peak_pos_last(m));
    mn2=min(mt2);
    q_peak(r_peak_pos_last(m)-(ceil(fs/bpm)*5)-1+find(mt2==mn2))= mn2;
end
...
for t=1:length(s_peak_pos_last)-1
    mt6=e4(s_peak_pos_last(t)+ceil(fs/bpm)*5:s_peak_pos_last(t)+ceil(fs/bpm)*30);
    mn6=max(mt6);
    t_peak(find(mt6==mn6)+s_peak_pos_last(t)+ceil(fs/bpm)*5-1)=mn6;
end
...
for t=1:length(t_peak_pos)
    mt9=ecgfiltered_last(t_peak_pos(t)-4:t_peak_pos(t)+ceil(fs/bpm));
    mn9=max(mt9);
    in = find(mt9==mn9);
    t_peak_last(in(1)+t_peak_pos(t)-5)=mn9;
end
...
for p=2:length(q_peak_pos_last)
    mt10=e4(q_peak_pos_last(p)- ceil((fs/bpm)*13):q_peak_pos_last(p));
    mn10=max(mt10);
    p_peak(find(mt10==mn10)+q_peak_pos_last(p)-ceil((fs/bpm)*13)-1)=mn10;
end
...
for p=2:length(p_peak_pos)
    mt11=ecgfiltered_last(p_peak_pos(p)- ceil((fs/bpm)*17):p_peak_pos(p)+ceil(fs/bpm)-1);
    mn11=max(mt11);
    in = find(mt11==mn11);
    p_peak_last(in(1)+p_peak_pos(p)-(ceil((fs/bpm)*17)+1))=mn11;
end
...
for i=1:length(t_peak_pos_last)
    mt18=ecgfiltered_last(t_peak_pos_last(i):t_peak_pos_last(i)+80);
    mn18=min(mt18);
    in = find(mt18==mn18);
...
% %     title(str);    
% end
% basefilename='ECG-TEB-2-öl-MCT+SIL+BOS-';
% dlmwrite([basefilename num2str(FileName([26 27 28])) '.csv'] ,[frekansdegeri_xpoz' max_genlik' band_freq_val' ortalamasure'],'precision','%20.5f');
